...
lcd.begin(16, 2);
lcd.print("Testing Battery Capacity");
...
void loop() {
  ...
  if (state == 0)  {
    ...
    if (batteryValue >= 294) {  //If the battery voltage is greater than or equal to 14.4V then
      ...
      state = 1;
      lcd.setCursor(0, 2);
      lcd.print(state);  //Change the state to 'resting' and exit the loop
    }
  }
  ...
  if (state == 1) {  //we're letting the battery rest for 30 seconds before testing it for early failure and / or moving on
    ...
    Serial.println(state);
    lcd.setCursor(0, 3);
    lcd.print(state);  //Lets see what state we're in
    ...
    lcd.setCursor(1, 1);
    lcd.print(voltage);  //Tell the world about that voltage
  ...
  if (state == 2) {
    ...
    if (voltage <= 12.5) {  //If the battery voltage is less than or equal to 12.5V then 
      ...
      Serial.println(state);
      lcd.setCursor(0, 1);
      lcd.print(state);  //Lets see what state we're in    Serial.println("TEST COMLPETE");
